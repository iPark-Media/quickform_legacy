...
// | http://www.php.net/license/2_02.txt.                                 |G
...
// +----------------------------------------------------------------------+
//
// $Id: QuickForm.php,v 1.3 2007/12/20 15:00:11 cvs_timo Exp $
...
class HTML_QuickForm extends HTML_Common {
    ...
    */
    ...
    /**
     * Löschen von Regeln für ein Formularelement
     *
     * Als default wird die required-Regel gelöscht
     *
     * @author Martin Kluge <m.kluge@ipark-media.de>
     * @param String $element_name
     * @param String $rule_type
     */
    function removeRule($element_name, $rule_type = false)
    {
        // Wenn keine spezielle Regel angegeben wurde, dann wird die required-Regel gelöscht
    	if($rule_type == false && array_search($element_name, $this->_required) !== false)
    	{
    		unset($this->_required[array_search($element_name, $this->_required)]);
    		$rule_type = 'required';
    	}

    	// Wird 'all' angegeben, werden alle Regeln gelöscht
    	if($rule_type == 'all')
    	{
            unset($this->_rules[$element_name], $this->_errors[$element_name]);
            if(array_search($element_name, $this->_required) !== false)
            {
        		unset($this->_required[array_search($element_name, $this->_required)]);
            }
    	}
    	elseif(is_array($rule_type))
    	{
    	    foreach($rule_type as $rule)
    	    {
        	    foreach($this->_rules[$element_name] as $rule_index => $rule_data)
        	    {
        	        if(strtolower($rule) == strtolower($rule_data['type']))
        	        {
        	            unset($this->_rules[$element_name][$rule_index], $this->_errors[$element_name]);
        	        }
        	    }
    	    }
    	}
    	elseif($rule_type != false)
    	{
    	    foreach($this->_rules[$element_name] as $rule_index => $rule)
    	    {
    	        if(strtolower($rule_type) == $rule['type'])
    	        {
    	            unset($this->_rules[$element_name][$rule_index], $this->_errors[$element_name]);
    	        }
    	    }
    ...
    */
    ...
    function freezeBlocks($blocks)
    {
        ...
        {
            $this->freezeBlock($block_name);
        }
    }

    /**
     * freezt alle Elemente eines Blocks
     *
     * @author Martin Kluge <m.kluge@ipark-media.de>
     * @param String $block_name
     */
    function freezeBlock($block_name)
    {
        foreach($this->_elements as $element)
        {

            $element_name = $element->getName();
            if(strpos($element_name, $block_name) !== false)
            {
                if($element->getType() == 'button' || $element->getType() == 'static')
                {
                    $this->removeElement($element_name);
                }
                else
                {
                    $this->freeze($element_name);
                }
            }
        }
    }
